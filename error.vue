<template>
  <div class="flex items-center w-full justify-center h-screen p-14">
    <div class="flex flex-col md:flex-row items-center">
      <div>
        <img src="./assets/images/error.png" alt="error-image" class="wobble" />
      </div>
      <div class="space-y-4 text-center md:text-left md:w-[400px]">
        <h1 class="font-bold text-5xl">Ooops, an error occurred</h1>
        <p class="text-xl">{{ error.message }}</p>
        <BaseButton
          @click="navigateTo('/')"
          buttonText="Go home"
          class="bg-purple-secondary p-3 rounded-md text-white font-bold hover:bg-purple-primary hover:text-purple-secondary duration-200"
        />
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
defineProps({
  error: Object,
});
</script>

<style scoped>
.wobble {
  animation: wobble 1s ease-in-out infinite alternate;
}

@keyframes wobble {
  from {
    transform: translateY(-10px);
  }
  to {
    transform: translateY(10px);
  }
}
</style>

<!-- <template>
  <div></div>
</template>

<script setup lang="ts"></script>

<style scoped></style> -->
